    ...
    for i, layer in enumerate(hparams.layers):
        ...
        if torch.cuda.device_count() == 1:
            upd_matrix = torch.linalg.solve(
                P[i,:,:].to("cuda") @ (layer_ks.to("cuda") @ layer_ks.T.to("cuda") + cache_c[i,:,:].to("cuda")) + hparams.L2*torch.eye(layer_ks.shape[0], dtype=torch.float,device="cuda"),
                P[i,:,:].to("cuda") @ layer_ks.to("cuda") @ resid.T.to("cuda")
            )
        else:
            upd_matrix = torch.linalg.solve(
