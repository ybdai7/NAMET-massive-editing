...
def compute_z(
    ...
    target_ids = tok(request["target_new"]["str"], return_tensors="pt").to(f"cuda:{hparams.device}")["input_ids"][0]
    ...
    lookup_idxs = [
        find_fact_lookup_idx(
            prompt, request["subject"], tok, hparams.fact_token, verbose=False, model_name=str(model.config._name_or_path).lower()
        )
