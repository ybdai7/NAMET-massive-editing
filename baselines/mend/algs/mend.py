...
class MEND(EditableModel):
    ...
    def edit(self, batch, condition=None, detach_history=False, return_factors=False, **kwargs):
        ...
        for i in range(0, batch[next(iter(batch.keys()))].size(0), max_batch_size):
            ...
            if 'minigpt4' in self.config.model_name.lower() or 'blip' in self.config.model_name.lower():
                cur_outputs = self.model(cur_batch)        
                if not isinstance(cur_outputs, torch.Tensor):
                    batch_labels = cur_outputs.labels
                    cur_outputs = cur_outputs.logits
                else:
                    batch_labels = cur_batch['labels']
                cur_loss = self.edit_loss_fn(self.config, cur_outputs, batch_labels, multimodal=True)["nll"]          
                cur_loss.backward()
            elif 'gpt' in self.config.model_name.lower():
                cur_outputs = _logits(self.model(input_ids=cur_batch['input_ids'], attention_mask=cur_batch['attention_mask']))
                # cur_outputs = cur_outputs[:, -cur_batch['labels'].shape[-1]:, :]
                if not kwargs:
                    cur_loss = self.edit_loss_fn(self.config, cur_outputs, cur_batch["labels"])["nll"]
                else:
                    cur_loss = self.edit_loss_fn(self.config, cur_outputs, cur_batch["labels"], **kwargs)["nll"]
                cur_loss.backward()
            elif 'llama' in self.config.model_name.lower():
                cur_outputs = _logits(self.model(input_ids=cur_batch['input_ids'], attention_mask=cur_batch['attention_mask']))
                # cur_outputs = cur_outputs[:, -cur_batch['labels'].shape[-1]:, :]
