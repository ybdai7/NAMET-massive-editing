...
def layer_stats(
    ...
    def get_ds():
        ...
        if "qwen" in str(model.config._name_or_path).lower() or \
    ...
    if "llama-3.1" in str(model.config._name_or_path).lower():
        batch_size = 100  # Examine this many dataset texts at once original 200
    else: 
        batch_size = 200  # Examine this many dataset texts at once original 200
    ...
    with torch.no_grad():
        for batch_group in progress(loader, total=batch_count):
            for batch in batch_group:
                ...
                # feats gives [#nonpaddingtokens, #features]
                feats = feats.to(dtype=dtype)
                stat.add(feats)
                # stat.add gives a.t().mm(a), which is [#features,#features]
    return stat
